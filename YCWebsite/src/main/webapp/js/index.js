$(function() {
	...
	$('#toggle_team span').mouseover(
			function() {
				if ($(this).index() == 0) {
					$(this).addClass('choice_team').siblings().removeClass(
							'choice_team');
					$('#teacher_team').show();
					$('#manage_team').hide()
				} else {
	...
	$('#project ul li').hover(
			function(e) {
				...
				var img = imgname.substring(3);
				var getleft = (e.pageX - $(this).offset().left)
						/ $(this).width();
				var gettop = (e.pageY - $(this).offset().top)
						/ $(this).height();
					...
					$('#showBigimg').css(
							{
								...
								'background-position' : (getleft * 100) + '% '
										+ (gettop * 100) + '%'
							}).show('normal')
				} else {
					$('#showBigimg').css(
	...
	$('#blog_content ul li').click(
			function() {
				var showname = $(this).attr('id') + '_show';
				$(this).addClass('current_article').siblings().removeClass(
						'current_article');
				$('#blog_content div').hide();
				$('#' + showname).show()
			});
	var picTimer;
	var actwidth = $('#act_content ul li').length
			* $('#act_content ul li').innerWidth();
			...
			.hover(
					...
					function() {
						picTimer = setInterval(
								function() {
									$('#act_content ul')[0].style.left = $('#act_content ul')[0].offsetLeft
											- 5 + 'px';
	...
	$('#course_content ul li').mouseover(
			function() {
				...
				$(this).addClass('course_current').siblings().removeClass(
						'course_current');
				$('#course_img img')[0].src = '../' + 'images/' + imgsrc
						+ '.png';
