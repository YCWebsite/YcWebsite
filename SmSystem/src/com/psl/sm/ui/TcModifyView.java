...
public class TcModifyView extends Dialog {
	 ...
	 * @param style
	 */
	public TcModifyView(Shell parent, int style) {
		super(parent, style);
		setText("套餐信息修改");
	}
	...
	private void createContents(final String[] str) {
		...
		text_3.setText(str[3]);

		Button button = new Button(shell, SWT.NONE);
		button.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Integer rid = Integer.parseInt(str[4]);
				String rname = text.getText().toString().trim();
				String rnote = text_1.getText().toString().trim();
				if(!"".equals(rname) & !"".equals(rnote)
				& !"".equals(text_2.getText().toString().trim()) & !"".equals(text_3.getText().toString().trim())){
					try{
						Float money_vip = Float.parseFloat(text_2.getText().toString().trim());
						Float money_pub = Float.parseFloat(text_3.getText().toString().trim());
					...
					MessageUtil.promt(shell, "警告", "请将修改信息填写完整！");
				}
			}
		});
